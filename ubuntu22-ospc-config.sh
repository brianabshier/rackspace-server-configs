#!/bin/bash
# v1.2 - User Prompt for Optional Installations
...
# Prompt user for Rackspace Monitoring Agent installation
read -p "Do you want to install the Rackspace Monitoring Agent? (Y/N): " INSTALL_MONITORING
INSTALL_MONITORING=${INSTALL_MONITORING,,}  # Convert to lowercase

if [[ "$INSTALL_MONITORING" == "y" ]]; then
    echo "Installing and configuring Rackspace Monitoring Agent..."
    ...
    echo "Skipping Rackspace Monitoring Agent installation."
fi

# Prompt user for Driveclient installation
read -p "Do you want to install the Backup Agent? (Y/N): " INSTALL_DRIVECLIENT
INSTALL_DRIVECLIENT=${INSTALL_DRIVECLIENT,,}  # Convert to lowercase

if [[ "$INSTALL_DRIVECLIENT" == "y" ]]; then
    echo "Installing and configuring Driveclient..."
    ...
    echo "Skipping Driveclient installation."
fi
